...
func ReconcileHop1(
	...
	for _, internal := range sortedInternals {
		...
		for _, s := range merchantSettlements {
			...
			refMatch := false
			if internal.ReferenceID != nil && s.ReferenceID != nil && *internal.ReferenceID == *s.ReferenceID {
				refMatch = true
			}
